...
$sql = 'SELECT u.id,u.email,u.auth_source,u.created_at,u.last_sign_in_at,u.last_seen_at,p.display_name,p.phone,p.whatsapp,p.date_of_birth,CASE WHEN p.date_of_birth IS NULL THEN NULL ELSE TIMESTAMPDIFF(YEAR,p.date_of_birth,CURDATE()) END age,p.gender,p.city,p.province_region,p.country,p.css_attempt_year,p.preparation_level,p.optional_subjects,p.education,p.previous_academy_mentor,p.profile_photo_bytes,p.profile_completed_at,br.id registration_id,br.registration_code,br.status registration_status,br.payment_status,br.submitted_at,b.id batch_id,b.title batch_title' . $join . ' WHERE ' . $where . ' ORDER BY COALESCE(p.last_seen_at,u.last_seen_at,u.last_sign_in_at,u.created_at) DESC LIMIT ' . $limit . ' OFFSET ' . $offset;
...
foreach ($rows as &$row) {
    ...
    $row['has_photo'] = !empty($row['profile_photo_bytes']);
}
unset($row);
