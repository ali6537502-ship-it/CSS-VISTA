...
$query=$pdo->prepare('SELECT id,email,password_hash,disabled_at,email_verified_at FROM users WHERE email=? LIMIT 1'); $query->execute([$email]); $user=$query->fetch();
...
if (!$user['email_verified_at']) cssv_fail('Confirm your email before signing in. You can request another confirmation link below.',403,'email_confirmation_required');
