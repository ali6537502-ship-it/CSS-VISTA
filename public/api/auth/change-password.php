...
$pdo=cssv_db(); account_auth_schema($pdo); $session=cssv_require_user($pdo, false); cssv_require_csrf($session);
...
try {
    ...
    $pdo->prepare('DELETE FROM account_reset_codes WHERE user_id=?')->execute([$session['user_id']]);
