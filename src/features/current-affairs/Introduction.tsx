...
export default function Introduction() {
  ...
  return <div className="ca-intro">
    <header className="ca-intro-hero"><p className="ca-eyebrow">CSS VISTA · DAILY AFFAIRS BRIEF</p><h1>Understand the day.<br />Remember what matters.</h1>
      <p className="ca-intro-lead">CSS Vista Daily Affairs Brief is a focused current-affairs reading desk: clear explanations, revision facts, important statistics and original sources, together in your free CSS Vista account.</p>
      <div className="ca-intro-cta"><Link className="ca-button" to={user ? '/account/dashboard' : '/account?mode=create&returnTo=/account/dashboard'}>{user ? 'Open My CSS Vista' : 'Create Free Account'} <ArrowRight size={17} /></Link>{!user && <Link className="ca-button ca-button-light" to="/account?returnTo=/account/dashboard">Log in</Link>}</div>
    </header>
    <div className="ca-intro-grid">{[
      ...
      ['06', 'Keep a clear view', 'Published Daily Affairs Brief editions bring verified-source material into one calm, organised place, with sources visible throughout.'],
