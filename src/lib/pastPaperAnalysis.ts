...
const subjectRequests = new Map<string, Promise<PastPaperAnalysisSubject>>()

/**
 * One subject's full analysis, including question text.
 *
 * The whole payload is ~1.8 MB and a visitor reads one subject at a time, so
 * the page pairs the compact index with a single subject file generated by
 * scripts/split-past-paper-analysis.mjs.
 */
export function loadPastPaperAnalysisSubject(slug: string) {
  const existing = subjectRequests.get(slug)
  if (existing) return existing

  const request = fetchJson<PastPaperAnalysisSubject>(`/css-past-paper-analysis/${slug}.json`)
    .catch(async (error) => {
      subjectRequests.delete(slug)
      // Fall back to the full payload if the split files are not deployed yet,
      // so the page keeps working against an older build output.
      const full = await loadPastPaperAnalysis().catch(() => null)
      const subject = full?.subjects.find((item) => item.slug === slug)
      if (subject) return subject
      throw error
    })

  subjectRequests.set(slug, request)
  return request
}
