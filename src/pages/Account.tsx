...
export default function Account() {
  ...
  async function emailAction(kind: 'reset' | 'verify') {
    ...
    try {
      ...
      else { if (kind === 'reset') setParams({ recovery: 'code' }); setMessage(kind === 'reset' ? 'If this account is eligible, a password-reset email will arrive shortly. Please also check your spam folder.' : 'If verification is still needed, a new email will arrive shortly. Please also check your spam folder.') }
    } finally { setBusy(false) }
  }
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:py-10">
      {loading ? <div className="vista-card flex items-center justify-center gap-2 p-10 text-sm text-muted-foreground" role="status"><LoaderCircle className="h-5 w-5 animate-spin" />Checking your account…</div>
        ...
        : <div className="grid gap-5 md:grid-cols-[1.05fr_.95fr]"><section className="vista-card p-5 sm:p-6">
          ...
          {codeRecovery && <button type="button" disabled={busy} onClick={() => void emailAction('reset')} className="mt-3 min-h-11 text-sm font-semibold text-pine underline">Send a new reset code</button>}
