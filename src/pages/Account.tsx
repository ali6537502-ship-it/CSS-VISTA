...
export default function Account() {
  ...
  async function submit(event: FormEvent) {
    ...
    setBusy(true)
    try {
      const result = reset ? await account.updatePassword(password, undefined, token)
      ...
      if (result.confirmationRequired) { setMessage('Check your inbox and spam folder for the email verification link, then sign in.'); setMode('sign-in'); return }
      navigate(safeReturnTo(params.get('returnTo')), { replace: true })
    } catch { setError('Please check your connection and try again.') }
    finally { setBusy(false) }
  }
  async function emailAction(kind: 'reset' | 'verify') {
    clear()
    if (!email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) { setError('Enter your email address first.'); return }
    setBusy(true)
    try {
      const result = kind === 'reset' ? await account.requestPasswordReset(email.trim()) : await account.resendVerification(email.trim())
      if (result.error) setError(result.error)
      else setMessage(kind === 'reset' ? 'If this account is eligible, a password-reset email will arrive shortly. Please also check your spam folder.' : 'If verification is still needed, a new email will arrive shortly. Please also check your spam folder.')
    } finally { setBusy(false) }
  }
  ...
  return <div>
    <PageHeader title="Your CSS Vista Account" description="Your free account brings daily current affairs, saved developments, revision facts and study progress together across devices." />
    <div className="mx-auto max-w-4xl px-4 py-8 sm:py-10">
      {loading ? <div className="vista-card flex items-center justify-center gap-2 p-10 text-sm text-muted-foreground" role="status"><LoaderCircle className="h-5 w-5 animate-spin" />Checking your account…</div>
        : verify ? <section className="vista-card mx-auto max-w-lg p-6"><h2 className="mb-3 text-xl font-bold text-pine">Verify your email</h2>{notices}{token ? <><p className="mb-5 text-sm text-muted-foreground">Confirm your email address to activate your free CSS Vista account.</p><button className={button} disabled={busy} onClick={() => void verifyAddress()}>{busy ? 'Verifying…' : 'Verify my email'}</button></> : <p>This verification link is incomplete. <Link className="font-semibold underline" to="/account">Return to sign in and request a new link.</Link></p>}</section>
        : user && !reset ? <div className="space-y-5">{notices}<div className="grid gap-5 md:grid-cols-2"><section className="vista-card p-6"><p className="text-xs font-bold uppercase tracking-wide text-emerald-700">Signed in</p><h2 className="mt-2 break-words text-2xl font-bold text-pine">{user.display_name || user.email.split('@')[0]}</h2><p className="mt-1 break-all text-sm text-muted-foreground">{user.email}</p><div className="mt-6 flex flex-wrap gap-3"><Link className={button} to="/account/dashboard">My Daily Briefing</Link><Link className={button} to="/dashboard">Study progress</Link><Link className="inline-flex min-h-11 items-center font-semibold text-pine underline" to="/account/settings">Account settings & password</Link><button className="min-h-11 rounded-md border px-4 font-semibold" disabled={busy} onClick={() => void signOut()}>{busy ? 'Signing out…' : 'Sign out'}</button></div></section><section className="vista-card p-6"><h2 className="text-lg font-bold text-pine">Progress sync</h2><p className="mt-3 text-sm text-muted-foreground">Your device keeps working offline. When connected, progress is merged into your private account.</p><button className={`${button} mt-4`} disabled={account.syncStatus === 'syncing'} onClick={() => void account.syncNow()}>{account.syncStatus === 'syncing' ? 'Syncing…' : 'Sync progress now'}</button>{account.lastSyncedAt && <p className="mt-3 text-xs text-muted-foreground">Last synced: {account.lastSyncedAt.toLocaleString()}</p>}{account.syncError && <p role="alert" className="mt-3 text-sm text-red-800">{account.syncError}</p>}</section></div><StudentProfilePanel email={user.email} /><AuthenticatedAccountAd sensitiveControlsVisible={reset || verify} /></div>
        : <div className="grid gap-5 md:grid-cols-[1.05fr_.95fr]"><section className="vista-card p-5 sm:p-6">
          ...
          {notices}
          {reset && !token ? <p className="text-sm">This reset link is incomplete. <Link className="font-semibold underline" to="/account">Request a new password-reset email.</Link></p> : <form onSubmit={(event) => void submit(event)} className="space-y-4">
            ...
            {!reset && <div className="flex flex-col"><button type="button" disabled={busy} className="min-h-11 text-sm font-semibold text-pine underline" onClick={() => void emailAction('reset')}>Forgot your password?</button><button type="button" disabled={busy} className="min-h-11 text-sm text-pine underline" onClick={() => void emailAction('verify')}>Resend verification email</button></div>}
          </form>}
        </section><aside className="vista-card h-fit p-5 sm:p-6"><h2 className="flex items-center gap-2 text-lg font-bold text-pine"><ShieldCheck className="h-5 w-5" />What your account saves</h2><ul className="mt-4 space-y-3 text-sm text-muted-foreground">{['Daily current affairs, original sources and revision facts', 'Saved developments and reading history', 'Your personal factbook', 'Quiz attempts, scores and study history', 'Saved MCQs, checklists and study tools'].map((item) => <li key={item} className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-700" />{item}</li>)}</ul><p className="mt-5 border-t pt-4 text-xs leading-relaxed text-muted-foreground">Account creation is free. Your existing guest study progress is merged after sign-in.</p></aside></div>}
    </div>
  </div>
}
