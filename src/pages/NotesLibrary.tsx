...
export default function NotesLibrary() {
  ...
  return (
    <div>
      <PageHeader title="Notes by Ali Hassan Sargana" description="Browse every authorised public sample, then contact the verified author for current availability and purchase details." />
      <main className="mx-auto max-w-6xl px-4 py-8 sm:py-10">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-2">
          {noteProducts.map((product) => (
            <article key={product.id} className="flex flex-col rounded-xl border bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
              <span className="w-fit rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-900">Authorised samples available</span>
              ...
              <div className="mt-4 space-y-2" aria-label={`${product.subject} public samples`}>
                {product.samples.map((sample) => (
                  <Link key={sample.id} to={`/notes/view/${product.id}/${sample.id}`} className="flex w-full items-center gap-2 rounded-lg border bg-secondary/35 px-3 py-2.5 text-left text-sm font-semibold text-pine hover:border-emerald-600 hover:bg-emerald-50">
                    ...
                    <span className="min-w-0 flex-1"><span className="block">{sample.title}</span><span className="mt-0.5 block text-xs font-normal text-muted-foreground">{sample.pages} pages · {formatFileSize(sample.sizeBytes)} · {sample.kind === 'pdf' ? 'PDF' : 'page viewer'}</span></span>
                    <Eye className="h-4 w-4 shrink-0 text-emerald-700" aria-hidden="true" />
                    <span className="sr-only">View complete public sample</span>
                  </Link>
                ))}
              </div>
              <p className="mt-4 flex-1 text-sm leading-relaxed text-foreground/80">Every uploaded page in each authorised sample is available through the viewer.</p>
        ...
        <p className="mt-6 flex items-start gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" /> Public samples are provided for review. Complete paid notes are not publicly downloadable and are supplied privately after purchase.</p>
      </main>
    </div>
  )
}
