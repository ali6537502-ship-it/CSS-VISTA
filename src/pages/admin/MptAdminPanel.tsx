...
import { pktDateTime } from '@/lib/mpt/copy'
...
type Stats = { applications: number; started: number; in_progress: number; completed: number; absent: number | null; average_score: number | null; completion_rate: number | null; distribution: Array<{ from: number; to: number; count: number }> }
type AdminMock = MptMock & { id: string; schedule_key: string | null; capacity: number | null; reserved_count: number; rank_min_candidates: number; scoring_version: number; cancel_reason: string | null; created_by: string; stats: Stats }
...
type AppRow = { candidate: string; email: string; candidate_code: string | null; roll_number: string; application_code: string; applied_at: string; status: string; phase: string; score: number | null }
...
function MockDetail({ mock, onChanged }: { mock: AdminMock; onChanged: () => void }) {
  ...
  useEffect(() => {
    if (tab === 'applications') get<{ rows: AppRow[]; total: number }>(`?view=applications&slug=${mock.slug}&q=${encodeURIComponent(search)}`).then(setApps).catch(() => setApps(null))
    if (tab === 'attempts') get<{ rows: AttemptRow[]; total: number }>(`?view=attempts&slug=${mock.slug}`).then(setAttempts).catch(() => setAttempts(null))
  }, [tab, mock.slug, search, version])
    ...
    <section className="rounded-xl border bg-white p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Exam {pktDateTime(mock.exam_open_at)} · entry closes {pktDateTime(mock.entry_close_at)} · ends {pktDateTime(mock.exam_end_at)}</p>
      ...
      <dl className="mt-3 grid grid-cols-2 gap-2 text-sm sm:grid-cols-4 lg:grid-cols-7">
        {[['Applications', s.applications], ['Started', s.started], ['In progress', s.in_progress], ['Completed', s.completed], ['Absent', s.absent ?? '—'], ['Average score', s.average_score ?? '—'], ['Completion', s.completion_rate === null ? '—' : `${s.completion_rate}%`]].map(([label, value]) => (
      ...
      {tab === 'applications' && (
        <div className="mt-3">
          <div className="flex flex-wrap gap-2">
            ...
            <a href={`/api/admin/mpt.php?view=applications&slug=${mock.slug}&format=csv`} className={`${button} inline-flex items-center`}>Export CSV</a>
          </div>
          <div className="mt-2 overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="text-xs uppercase text-muted-foreground"><tr><th className="py-1">Candidate</th><th>Roll No.</th><th>Application ID</th><th>Applied</th><th>State</th><th>Score</th><th /></tr></thead>
              <tbody className="divide-y">
                {apps?.rows.map((row) => (
                  <tr key={row.application_code}>
                    ...
                    <td className="text-xs">{pktDateTime(row.applied_at)}</td><td className="text-xs">{row.phase}</td><td>{row.score ?? '—'}</td>
      ...
      {tab === 'settings' && (
        <form onSubmit={save} className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {field('close_offset_minutes', 'Applications close (minutes after start)')}
          {field('entry_close_offset_minutes', 'Entry closes (minutes after start)')}
        ...
        <>
          <div className="grid gap-3 text-sm sm:grid-cols-3">
            ...
            <div className={`rounded-xl border p-3 ${overview.runway.available <= 4 ? 'border-amber-300 bg-amber-50' : 'bg-white'}`}>
              ...
              {!overview.runway.publishable && <p className="text-xs font-semibold text-amber-800">Papers are not publishable: set CSSV_MPT_PAPER_SEED in the Hostinger build environment and redeploy.</p>}
          ...
          <form className="flex flex-wrap items-end gap-2 rounded-xl border bg-white p-3" onSubmit={(event) => {
            ...
            <p className="w-full text-xs text-muted-foreground">Defaults: applications open 24 h before, close 10 min after start; 200 minutes; Roll Number after 10 minutes.</p>
            {create.message && <p role="status" className="w-full text-sm">{create.message}</p>}
          </form>

          <div className="overflow-x-auto rounded-xl border bg-white">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="bg-secondary/40 text-xs uppercase text-muted-foreground"><tr><th className="px-3 py-2">Mock</th><th>Exam (PKT)</th><th>Status</th><th>Applied</th><th>Completed</th><th>Avg</th><th /></tr></thead>
              <tbody className="divide-y">
                {overview.mocks.map((mock) => (
                  <tr key={mock.slug} className={selected === mock.slug ? 'bg-emerald-50' : ''}>
                    ...
                    <td>{mock.stats.applications}{mock.capacity !== null ? ` / ${mock.capacity}` : ''}</td><td>{mock.stats.completed}</td><td>{mock.stats.average_score ?? '—'}</td>
