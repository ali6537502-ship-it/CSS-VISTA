...
import { Sparkles, BookOpen, Flame, Landmark, Mountain, Orbit, Play, Target, Waves, Clock3 } from 'lucide-react'

const THEMES = [
  { id: 'video', number: '01', name: 'Cinematic Green Flow', summary: 'A real muted video loop, compressed to about 2 MB with an automatic lightweight fallback.', icon: Play },
  { id: 'aurora', number: '02', name: 'Emerald Aurora', summary: 'Slow cinematic light, soft particles and a calm premium atmosphere.', icon: Sparkles },
  { id: 'orbit', number: '03', name: 'Knowledge Orbit', summary: 'Living orbital rings represent subjects moving around one clear goal.', icon: Orbit },
  { id: 'horizon', number: '04', name: 'Pakistan Horizon', summary: 'A moving night horizon with a subtle national and aspirational identity.', icon: Mountain },
  { id: 'pages', number: '05', name: 'Living Knowledge', summary: 'Floating pages and softly moving study lines create an academic mood.', icon: BookOpen },
  { id: 'momentum', number: '06', name: 'Momentum Waves', summary: 'Continuous flowing lines make progress feel active and forward-moving.', icon: Waves },
] as const
...
function Wallpaper({ theme }: { theme: ThemeId }) {
  if (theme === 'video') {
    const allowVideo = !((navigator as Navigator & { connection?: { saveData?: boolean } }).connection?.saveData) && !window.matchMedia('(prefers-reduced-motion: reduce)').matches
    return (
      <div className="live-wallpaper live-wallpaper-video" aria-hidden="true">
        {allowVideo && (
          <video className="live-wallpaper-video-media" autoPlay muted loop playsInline preload="metadata">
  ...
  if (theme === 'aurora') {
    return (
      <div className="live-wallpaper live-wallpaper-aurora" aria-hidden="true">
        ...
        {PARTICLES.map((p, i) => (
          <i key={i} className="live-particle" style={{ left: p.left, top: p.top, animationDelay: p.delay, animationDuration: p.duration }} />
        ))}
      </div>
    )
  }
    ...
    return (
      <div className="live-wallpaper live-wallpaper-horizon" aria-hidden="true">
        ...
        {PARTICLES.slice(0, 12).map((p, i) => (
          <i key={i} className="live-star" style={{ left: p.left, top: p.top, animationDelay: p.delay }} />
        ))}
    ...
    return (
      <div className="live-wallpaper live-wallpaper-pages" aria-hidden="true">
        {Array.from({ length: 8 }, (_, i) => (
          <div key={i} className="live-page" style={{ left: `${6 + ((i * 14) % 88)}%`, top: `${5 + ((i * 23) % 70)}%`, animationDelay: `${i * -1.1}s` }}>
            <span /><span /><span />
          </div>
        ))}
...
function DemoStage({ theme }: { theme: ThemeId }) {
  return (
    <section className={`live-demo-stage live-demo-${theme}`}>
      ...
      <div className="relative z-10 flex min-h-[520px] flex-col p-5 text-white sm:p-8 lg:min-h-[560px] lg:p-10">
        <div className="flex items-center justify-between gap-3">
          <img src="/images/logo.png?v=20260810" alt="CSS Vista official logo" className="h-12 w-auto rounded-md bg-white/95 object-contain px-2 sm:h-14" />
        ...
        <div className="mt-auto grid items-end gap-6 lg:grid-cols-[1fr_420px]">
          <div className="max-w-2xl">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-amber-300/30 bg-amber-300/10 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.16em] text-amber-200">
              <Sparkles className="h-3.5 w-3.5" /> Your preparation is alive
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <button className="inline-flex h-11 items-center gap-2 rounded-lg bg-white px-5 text-sm font-bold text-emerald-950 shadow-lg transition-transform hover:-translate-y-0.5">
                <Play className="h-4 w-4 fill-current" /> Continue preparation
              </button>
...
export default function LiveThemeDemos() {
  const [active, setActive] = useState<ThemeId>('video')
          ...
          {THEMES.map((t) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl border p-3 text-left transition-all ${isActive ? 'border-emerald-700 bg-pine text-white shadow-lg' : 'border-emerald-900/10 bg-white text-pine hover:-translate-y-0.5 hover:border-emerald-700/35 hover:shadow-md'}`}
              >
                <div className="flex items-center justify-between gap-2">
                  <Icon className={`h-5 w-5 ${isActive ? 'text-amber-300' : 'text-emerald-800'}`} />
                  <span className={`text-[11px] font-bold ${isActive ? 'text-white/50' : 'text-muted-foreground'}`}>{t.number}</span>
                </div>
                <p className="mt-3 text-sm font-bold">{t.name}</p>
                <p className={`mt-1 hidden text-[11px] leading-relaxed sm:block ${isActive ? 'text-white/65' : 'text-muted-foreground'}`}>{t.summary}</p>
              </button>
            )
          })}
        </div>
        <DemoStage key={theme.id} theme={theme.id} />
        <div className="mt-4 rounded-xl border bg-white px-4 py-3 text-sm text-muted-foreground">
          <span className="font-bold text-pine">{theme.number}. {theme.name}:</span> {theme.summary} This is live motion, not a static image.
          {theme.id === 'video' && (
            <>
              {' '}Preview footage from{' '}
              <a className="font-semibold text-emerald-800 underline underline-offset-2" href="https://www.pexels.com/video/flowing-green-particles-12987971/" target="_blank" rel="noreferrer">Pexels</a>.
            </>
          )}
        </div>
      </div>
