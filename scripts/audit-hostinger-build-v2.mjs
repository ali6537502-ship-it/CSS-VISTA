...
assert(/<h1\b[^>]*>[\s\S]*Everything you need to prepare\.[\s\S]*One platform\. Completely free\.[\s\S]*<\/h1>/.test(indexHtml), 'Homepage H1 does not contain the approved preparation message')
...
assert(!indexHtml.includes('css-vista-main-poster-480.webp'), 'Retired homepage poster is still fetched by the text-first hero')
assert(indexHtml.includes('Create my free account'), 'Homepage first paint is missing the free student account action')
