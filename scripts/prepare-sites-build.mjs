...
function css2026ResultBody() {
  return `<main class="mx-auto max-w-4xl px-4 py-12"><p class="text-xs font-bold uppercase tracking-wide text-emerald-700">FPSC written result · announced ${escapeHtml(css2026Result.announcedDateLabel)}</p><h1 class="mt-2 font-display text-4xl font-bold text-pine">${escapeHtml(css2026Result.title)}</h1><p class="mt-4 text-base leading-relaxed text-muted-foreground">View and download the complete ${css2026Result.pageCount}-page list of ${css2026Result.qualifiedCandidates} candidates who qualified the written portion of the CSS Competitive Examination 2026.</p><dl class="mt-6 grid gap-3 rounded-xl border bg-white p-5 sm:grid-cols-3"><div><dt class="text-xs text-muted-foreground">Announced</dt><dd class="font-bold text-pine">${escapeHtml(css2026Result.announcedDateLabel)}</dd></div><div><dt class="text-xs text-muted-foreground">Qualified candidates</dt><dd class="font-bold text-pine">${css2026Result.qualifiedCandidates}</dd></div><div><dt class="text-xs text-muted-foreground">Document</dt><dd class="font-bold text-pine">${css2026Result.pageCount} pages · ${escapeHtml(css2026Result.fileSizeLabel)}</dd></div></dl><div class="mt-5 flex flex-wrap gap-3"><a href="${escapeHtml(css2026Result.pdfUrl)}" class="rounded-lg bg-pine px-4 py-3 text-sm font-bold text-white">View result PDF</a><a href="${escapeHtml(css2026Result.pdfUrl)}" download="${escapeHtml(css2026Result.downloadFilename)}" class="rounded-lg border px-4 py-3 text-sm font-bold text-pine">Download result</a></div><section class="mt-8"><h2 class="font-display text-2xl font-bold text-pine">How to check the CSS 2026 result</h2><p class="mt-2 text-sm leading-relaxed text-muted-foreground">Open the PDF and use its Find or Search command to locate a six-digit roll number or candidate name. This is the written-portion result, not the final allocation result.</p><a class="mt-4 inline-block font-bold text-emerald-800 underline underline-offset-2" href="${escapeHtml(css2026Result.officialResultsUrl)}">Verify updates on the FPSC results page</a></section><section class="mt-8"><h2 class="font-display text-2xl font-bold text-pine">What happens after the written result</h2><p class="mt-2 text-sm leading-relaxed text-muted-foreground">Qualifying the written examination is not the final stage. Candidates on this list proceed to the psychological assessment and the viva voce, which together carry their own marks and are conducted separately by the Commission. Final selection and allocation follow only after those stages are complete, so this list does not determine an occupational group.</p><p class="mt-3 text-sm leading-relaxed text-muted-foreground">Candidates who did not qualify this time can use the papers actually set in this cycle to see where the demand fell, before deciding what to change in the next attempt.</p><ul class="mt-4 grid gap-2 rounded-xl border bg-white p-5 text-sm sm:grid-cols-2"><li><a class="font-semibold text-emerald-800 underline underline-offset-2" href="/psych-viva">Psychological assessment and viva preparation</a> — the stages that follow this result</li><li><a class="font-semibold text-emerald-800 underline underline-offset-2" href="/past-papers/css/2026">CSS 2026 papers</a> — every paper set in this examination cycle</li><li><a class="font-semibold text-emerald-800 underline underline-offset-2" href="/css-past-paper-analysis">Topic-wise past-paper analysis</a> — what each subject has repeatedly asked</li><li><a class="font-semibold text-emerald-800 underline underline-offset-2" href="/services">CSS occupational groups</a> — the services allocation eventually leads to</li></ul></section><section class="mt-8 rounded-xl border border-amber-300 bg-amber-50/70 p-5"><h2 class="font-display text-xl font-bold text-pine">About this copy of the result</h2><p class="mt-2 text-sm leading-relaxed text-foreground/80">CSS Vista is not FPSC. This page hosts a copy of the result document${css2026Result.noticeNumber ? ` issued under notice ${escapeHtml(css2026Result.noticeNumber)}` : ''} so it can be found and searched easily. The Commission&rsquo;s own publication is the authoritative record, and a later correction or revised notice would appear there first.</p></section></main>`
}
