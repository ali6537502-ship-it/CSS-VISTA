...
if (process.argv.includes('--artifact')) {
  ...
  assert(sample.includes('Open and download the CSS 2023 Current Affairs Past Paper'), 'Direct paper HTML must provide a neutral PDF description')
  assert(!/owner-provided|watermark/i.test(sample), 'Direct paper HTML must not expose archive-processing labels')
  assert(!sample.includes('Verified questions recovered') && !sample.includes('Topic-wise analysis'), 'Direct paper HTML must not include analysis or reconstructed questions')
