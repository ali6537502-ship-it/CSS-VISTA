...
`/account` supports immediate registration, login, password recovery and logout. New student accounts do **not** require an email verification code or link: registration creates the account and authenticated session in the same protected request. Legacy `/account?verify=1#token=…` links remain accepted for older messages, but verification is no longer an activation requirement. `/account?reset=1#token=…` resets a password without requiring an existing session. The browser removes token fragments after reading them. The same Hostinger recovery email includes a six-digit code entered at `/account?recovery=code`. Recovery codes expire after 10 minutes, allow five guesses, are HMAC-hashed in `account_reset_codes`, and become unusable after either a code or link reset. Password changes invalidate both recovery methods. Recovery tokens are random, single use and stored as HMAC hashes; password-reset links expire after 30 minutes. New passwords contain 8–72 UTF-8 bytes. Existing migrated passwords remain verifiable using PHP's portable bcrypt/Argon verification. Password changes require the current password; resets and changes revoke previous sessions.
...
Configure a real Hostinger mailbox using `CSSV_SMTP_HOST`, `CSSV_SMTP_PORT`, `CSSV_SMTP_ENCRYPTION` (`ssl` or `tls`), `CSSV_SMTP_USER`, `CSSV_SMTP_PASSWORD`, `CSSV_MAIL_FROM` and `CSSV_MAIL_FROM_NAME`. Production password-recovery email and legacy verification resend require authenticated SMTP; PHP `mail()` is deliberately unavailable in production because its success value does not prove that Hostinger accepted or delivered the message. The sender address must match the authenticated mailbox.

The MFA-protected **Admin → Account emails** screen can connect the mailbox without placing its password in GitHub or frontend code. It tests the supplied credentials against `smtp.hostinger.com` first, sends a confirmation to the owner address, atomically updates the private configuration outside `public_html`, then retries up to ten queued account messages. The form never returns the saved password to the browser. Supported Hostinger combinations are SSL/465 and TLS/587.

Registration does not depend on mail transport. If SMTP is unavailable, password recovery and legacy verification resend return `email_service_unavailable` with HTTP 503 before looking up an account. This preserves account-enumeration protection while allowing new students to create accounts without consuming outgoing-mail quota. The public health response exposes only `configured.mail_ready` and `configured.smtp`; it never exposes credentials.

`server/sql/006_native_account_mail.sql` adds `account_verification_tokens`, `account_reset_codes` and `account_mail_outbox`; the API also creates them idempotently. Verification-token support is retained only for legacy links. The outbox encrypts addresses and token-bearing bodies with AES-256-GCM under the existing application secret. Accepted, cancelled and expired messages lose the encrypted body. Failed messages retry up to five times, at least two minutes apart. Recovery and legacy verification-resend requests drain the queue, and the existing admin panel's **Account emails** area shows aggregate status and can retry pending messages.
...
The disposable GitHub Actions database runs native registration with an immediate authenticated session and no signup email → duplicate-account rejection → login → optional legacy verification resend/replay protection → password recovery → captured reset email → reset → old-session rejection → password change. It also checks private Factbook CRUD/search/revisions/media, cross-user denial, rollback, persistence, the owner CMS, current affairs ingestion and signed migration. Test mail is restricted to `example.invalid`, stays in the disposable runner and is never uploaded as an artifact. Production checks are read-only and must not insert fabricated news.

## Mandatory profile completion

All 12 access checks are required: photo, full name, one contact number (phone or WhatsApp), date of birth, gender, city, province/region, country, attempt year, preparation level, optional subjects and education. `public/api/_profile.php` evaluates saved values, not a client flag or a historic completion timestamp. Clearing a required value relocks services. Profile editing/photo retrieval, password security, session checks and logout remain accessible while incomplete. Current affairs, private Factbook/media, synced progress, reports and test-series proposals enforce the gate on the server. The account dashboard, personal Factbook and signed-in study dashboard show profile setup until complete. Public study pages remain available.

The profile also records structured previous CSS Vista history. A student must state whether they previously studied with **Miss Sadia Zahoor**, **Sir Ali Hassan Sargana**, **both**, or **neither**. When applicable, the profile records whether the student joined a **Batch**, **Test Series**, or **Purchased Notes**, plus optional details. These fields are available to the protected admin student directory, its filters and exports. The older generic previous academy/mentor field remains optional for backward compatibility.

The profile screen provides a clickable 12-check checklist, three sections, saved completion status and photo preview. Profile photos are accepted only as validated JPG, PNG or WebP images and the private server-side hard limit is **60 KB (61,440 bytes)**. The browser and backend enforce the same 60 KB limit, and the photo-compressor page can prepare a compliant image.
