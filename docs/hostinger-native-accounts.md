...
Until SMTP is connected, registration, verification resend and password recovery return `email_service_unavailable` with HTTP 503 before looking up an account. This avoids creating unusable new accounts and preserves account-enumeration protection. The public health response exposes only `configured.mail_ready` and `configured.smtp`; it never exposes credentials.
