...
if (($argv[1] ?? '')==='user') {
    ...
    $pdo->prepare("INSERT INTO users(id,email,password_hash,auth_source,email_verified_at) VALUES(?,?,?,'local',NOW(6))")->execute([$id,$email,password_hash('TEST ONLY native fixture password',PASSWORD_BCRYPT)]);
