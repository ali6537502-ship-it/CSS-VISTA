...
for (const [label, viewport] of [['mobile', { width: 375, height: 800 }], ['desktop', { width: 1280, height: 900 }]]) {
  ...
  await page.getByLabel('Enter your 6-digit Roll Number').fill(roll.slice(0, 5) + String((Number(roll[5]) + 1) % 10))
  await page.getByRole('button', { name: 'Verify & Enter' }).click()
  await page.getByRole('alert').filter({ hasText: 'one digit looks wrong' }).waitFor()
  await page.getByLabel('Enter your 6-digit Roll Number').fill(`${roll.slice(0, 3)} ${roll.slice(3)}`)
  await page.getByRole('button', { name: 'Verify & Enter' }).click()
  ...
  await page.screenshot({ path: `${shots}/${label}-07-result.png`, fullPage: true })

  // Section 19 widths: no horizontal scroll on the candidate's pages.
  if (label === 'mobile') {
    for (const width of [320, 360, 390, 414]) {
      await page.setViewportSize({ width, height: 800 })
      for (const path of [`/account/mpt/results/${code}`, `/account/mpt/applications/${code}`, '/account/mpt', '/account/mpt/history', '/account/mpt/performance']) {
