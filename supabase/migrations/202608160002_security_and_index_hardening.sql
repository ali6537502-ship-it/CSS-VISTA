-- Resolve actionable database-advisor findings without weakening RLS.
...
create index if not exists mcq_error_reports_user_idx
  on public.mcq_error_reports (user_id);
create index if not exists mcq_error_reports_resolved_by_idx
  on public.mcq_error_reports (resolved_by);
...
using (
  ((select auth.uid()) = user_id and status = 'submitted')
  or public.is_css_vista_admin()
)
with check (
  ((select auth.uid()) = user_id and status = 'submitted')
  or public.is_css_vista_admin()
